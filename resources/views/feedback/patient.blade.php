...
@section('title', 'Patient Feedback & Treatment Sessions')
...
<div class="container">
    ...
    <form method="POST" action="{{ url('/feedback/patient-submit') }}">
        @csrf

        <!-- Hidden fields -->
        <input type="hidden" name="sessionsid" value="{{ $session->id }}">
        <input type="hidden" name="doctorid" value="{{ $session->doctor_id }}">
        <input type="hidden" name="patientid" value="{{ $session->patient?->id }}">

        <div class="mb-3">
            <label>Remarks:</label>
            <textarea name="patient_remarks" rows="4" class="form-control" required>{{ old('patient_remarks') }}</textarea>
        </div>

        <div class="mb-3">
            <label>Satisfaction (%):</label>
            <input type="number" name="satisfaction" min="0" max="100" class="form-control" value="{{ old('satisfaction') }}" required>
        </div>
    ...
    {{-- Treatment Slip & Sessions Section --}}
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            <div class="p-3 border rounded bg-light d-flex align-items-center">
                                ...
                                <div>
                                    <strong>Invoice:</strong> {{ $session->id }}
                                </div>
                            </div>
                        </div>
                                                    ...
                                                    <div class="card shadow-sm mb-3 border-0">
                                                        <div class="card-body p-3">
                                                            <div class="d-flex justify-content-between align-items-center">
                                                                ...
                                                                <span class="badge {{ $time->is_completed ? 'bg-success' : 'bg-warning text-dark' }}">
                                                                    {{ $time->is_completed ? 'Completed' : 'Pending' }}
                                                                </span>
                                                            </div>
                                                            <p class="mb-1 text-muted small">
                                                                <i class="material-icons-outlined fs-6 me-1">schedule</i>
                                                                {{ format_date($time->session_datetime) }} at {{ format_time($time->session_datetime) }}
                                                            </p>
                                                            <p class="mb-2 small">
